        ...
        <form id="sampleForm" >
            <div class="span4">
                ...
                <div class="well well-small">
                    ...
                    <!--     <button title="{MESA_SAD_TOOLTIPLOCKCLIENT}" id="lockClientButton" type="button" tabindex="-1" class="btn btn-mini pull-right"><i id="clientLockIcon" class="icon-unlock"></i> {MESA_SAD_CLIENT}</button> -->
                </div>
                ...
                <div class="well well-small">
                    <h6><i class="icon-suitcase"></i> {MESA_SAD_PROJINFORMATION} </h6>
                        ...
                        <span id="project_field_span" class="hide">
                            {project_fields}
                            {pj_project_extra_client_reference}
                        </span>
                    ...
                    <div class="tabbable tabs-below">
                        <div class="tab-content">
                            ...
                            <div class="tab-pane " id="profile">
                                ...
                                <div id="custProfileList" class="list-group hidden">
                                    {cust_profile_list}
                                </div>
                            ...
                            <div class="tab-pane active" id="assay">
                        ...
                        <ul class="nav nav-tabs">
                            <li class=""><a href="#profile" data-toggle="tab" tabindex="-1" ><i class="icon-sitemap"></i> {MESA_SAD_PROFILE}</a></li>
                            <li class="active"><a href="#assay" data-toggle="tab" tabindex="-1" ><i class="icon-random"></i> {MESA_SAD_ASSAYS}</a></li>
                            <!-- <li class=""><a href="#quickAdd" data-toggle="tab" tabindex="-1" ><i class="icon-bolt"></i> </a></li> -->                        
                            <li id="searchButtonAssays" class="hidden"><a href="#search" data-toggle="tab" tabindex="-1"  ><i class="icon-search"></i> </a></li>
...
$(function(){
    ...
    * =============   */
    ...
    $('#sampleForm').on('change', '.project_extra_field', function(){
...
function validateAssayRequest(selectedAssay){
    ...
    roamingAssaySelected = $(selectedAssay).attr('assayId');
     ...
     $.ajax({
        type: "POST",
        data: {requestedId: roamingAssaySelected, requestedType: 'assay', alreadyAssigned: reqResearch, excluded: excResearch},
        dataType: 'json',
        url: "{LB}/assayProfiles/validateAssayRequest"
    }).done(function(response) {
        ...
        if(response['valid'] == true){
           requestRoamingAssay(selectedAssay);
        } else{
...
function requestRoamingAssay(selectedAssay, editId){
    ...
    if(editId !== undefined ){
     ...
     $.ajax({
        ...
        url: "{LB}/samples/generateRoamingForm/" + roamingAssaySelected
     }).done(function(msg) {
         $('#roamingFormSpan').html(msg);

         //$('#roam_dillutionCG').hide();
         //$('#roam_replicatesCG').hide();
         ...
         if($(selectedAssay).attr('dillution') == '1'){
         ...
         if($(selectedAssay).attr('replicates') == '1'){
...
function saveRoamingAssay(){
    ...
    //var dillutionSet = $('#roam_dillution').val();
    //var replicateSet = $('#roam_replicates').val();
    //var referenceSet = $('#roam_reference').val();
    //var refscopeEntered = $('#roam_reference_scope').val();
    ...
    /* roamingAssayValues[researchCounter] = { assay_id: roamingAssaySelected,
                                            dillution: dillutionSet,
                                            replicates: replicateSet,
                                            reference: referenceSet,
                                            reference_scope: refscopeEntered
                                          };
      */
...
function loadClientProjects(preselect){
    $.ajax({
        type: "POST",
        url: "{LB}/projects/getProjectSelect/" + selected_client + "/" + selected_subclient + "/1"
    }).done(function(options) {
...
function saveSample(){
    ...
    $('input, select, textarea').each(function(){
            ...
            else if ($(this).hasClass('project_extra_field')){
                projectExtraFields.push({preloadFieldId: fieldId, preloadFieldVal: fieldValue});                
            } 
...
function editFromProfile(profileId, assayId, follow_no, assayOrigin){
    ...
    //show custom
    var assayObject = $('#singleassay_' + assayOrigin);
