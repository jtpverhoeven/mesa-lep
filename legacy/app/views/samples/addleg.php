                ...
                <div class="well well-small">
                    ...
                    <table class="table table-condensed table-bordered">
                        ...
                        <tr>
                            ...
                            <th width="150px;">Type water/test</th>
                            <th width="150px;">Monstername</th>
                            <th width="150px;">Monsternummer (indicatief)</th>
                        </tr>
                        </thead>
...
<script>
    ...
    $(function() {
        ...
        $('#sampleTableBody').on('change', '.leg-sample-profile', function(){
            redrawBarcodes();
        });
    ...
    function redrawBarcodes(){
        ...
        $('#sampleTableBody tr').each(function(){
            ...
            var matrixType = $(this).find('.leg-sample-profile').find(':selected').data('matrix-type');            
            samplePackage.push({ sampleI: sampleI, matrixType: matrixType });
        });        
        ...
        $.ajax({
            type: "POST",            
            dataType: "json",
            data: { samplePackage: samplePackage},
            url: "{LB}/samples/redrawLegBarcodes",
        }).done(function (response) {
            $.each(response, function(index, itemData) {
                $('#barcode_' + itemData.sampleI).val(itemData.barcode);
            });            
        });
    ...
    function saveSample() {
        ...
        $('#sampleTableBody tr').each(function(){
            ...
            var thisProfile = $('#profile_select_' + sampleI).val();
            var thisSamplingMethod = $('#sampling_method_' + sampleI).val();
            var thisMatrix = $(this).find('.leg-sample-profile').find(':selected').data('matrix-type');
            samplePackage.push({ thisFollow: thisFollow, thisProfile: thisProfile, thisSamplingMethod:thisSamplingMethod, thisMatrix: thisMatrix });
        });
    ...
    function loadSamplesForm(numberOfSamples){
        ...
        $.ajax({
            type: "POST",
            url: "{LB}/samples/renderAddLegSamples/" + numberOfSamples + "/" + selectedProject
        }).done(function(msg) {
            $('#nowLoadingIcon').hide();
            $('#sampleTableBody').html(msg);
        });
    }
