...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="lims-shell">
        ...
        <div class="workspace">
            @auth
                <aside class="sidebar">
                    <div class="sidebar-heading"><span class="eyebrow">Werkruimte</span><strong>{{ request()->is('beheer*') ? 'Beheer' : 'LIMS' }}</strong></div>
                    <nav aria-label="Werkruimtenavigatie">
                        @if(request()->is('beheer*'))
                            ...
                            @can('settings.advanced')<a href="{{ route('cvars.index') }}" @class(['selected' => request()->is('beheer/cvars*')])>Geavanceerde instellingen</a>@endcan
                        @else
                            ...
                            <span class="nav-group"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-2px;margin-right:4px" aria-hidden="true" focusable="false"><path d="M18 21a8 8 0 0 0-16 0"></path><circle cx="10" cy="7" r="4"></circle><path d="M22 21a8 8 0 0 0-6-7.75"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path></svg>Klanten</span>
                            @can('clients.view')<a href="{{ route('clients.index') }}" @class(['selected' => request()->is('laboratorium/klanten*')])>Klantenoverzicht</a>@endcan
                            @can('clients.view')<a href="{{ route('client-categories.index') }}" @class(['selected' => request()->is('laboratorium/klantcategorieen*')])>Klantcategorieen</a>@endcan
