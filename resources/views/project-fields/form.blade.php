<fieldset class="form-section"><legend>Veldgegevens</legend>
    ...
    <div class="field"><label for="type">Type</label><select id="type" name="type" required><option value="text" @selected(old('type', $field?->type) === 'text')>Tekst</option><option value="date" @selected(old('type', $field?->type) === 'date')>Datum</option></select></div>
    ...
    <input type="hidden" name="keep_current" value="0"><label><input type="checkbox" name="keep_current" value="1" @checked(old('keep_current', $field?->keep_current ?? 1))> Huidige waarde behouden</label>
</fieldset>